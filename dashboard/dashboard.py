...
def create_sum_order_items_df(df):
    products_count = df.groupby("product_category_name_english").product_id.count().reset_index()
    sum_products = products_count.sort_values(by="product_id", ascending=False)
...
def create_sum_rating_df(df) :
    rating_count = df["review_score"].value_counts()
    return rating_count
...
main_df = main_data_df[(main_data_df["order_purchase_timestamp"] >= str(start_date)) & 
...
sum_rating_df = create_sum_rating_df(main_df)
rfm_df = create_rfm_df(main_df)

# plot number of daily orders
st.header('E-Commerce Data Analyst Dashboard :sparkles:')
...
# Product performance
st.subheader("Best & Least Sold Products")
...
with col1:
    ...
    sns.barplot(x=sum_rating_df.index,
                y=sum_rating_df.values,
                order=sum_rating_df.index,
                palette=colors)
    ax.set_title("Rating Customers for Each Order", fontsize=15)
    ax.set_xlabel(None)
    ax.set_ylabel(None)
    ax.tick_params(axis='y', labelsize=20)
    ax.tick_params(axis='x', labelsize=15)
